...
电子班牌及考勤机的主程序用C#语言编写，请假页面是网页。主程序中主体一致，三种机器的程序差别主要在Addons目录下的插件中。该程序相当臃肿，代码据说相当复杂，程序目录下的日志也经常从2018年刚装上那会积攒到现在。
...
电子班牌配置的手动IP顺序大致为：远志楼→铭志楼→远翔楼→通慧楼→六艺楼→宿舍→校门。各活动教室（按前几届时教室的排布）、办公室、阶梯教室、机房等的IP在约150至240间穿插。
